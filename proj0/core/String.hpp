...
private:
    ...
    char* c;    // char array of string
    unsigned int cap;   //capacity of the char array

    // copy a source string (an existing array) to a "this" string
    void copyArr(const String& source); 

    // copy a source array of char to a target char array
    void copy(char* charArr, const String& source, unsigned int leng);

    // return true if all the elements of char array is same
    bool compareChar(String s1, String s2) const;
};
